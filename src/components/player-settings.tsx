...
export function PlayerSettings({value,onChange}:{value:Preferences;onChange:(v:Preferences)=>void}){
 ...
 async function save(patch:Partial<Preferences>){
  ...
  try{const r=await fetch('/api/player/preferences',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(patch),signal:AbortSignal.timeout(10000)});if(!r.ok)throw Error();onChange(await r.json() as Preferences);}
 ...
 <section><h3>{t.font}</h3><div className="font-options" role="group" aria-label={t.font}>{FONTS.map(font=><button type="button" key={font} data-font={font} aria-pressed={value.font===font} disabled={busy} onClick={()=>void save({font})}>{font.charAt(0).toUpperCase()+font.slice(1)}</button>)}</div></section>
