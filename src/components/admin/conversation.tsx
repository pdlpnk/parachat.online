...
export function AdminConversation({ id, initial, onUpdate, onBack, onTags }: { id: string; initial: AdminDetail; onUpdate: (conversation: ConversationDTO) => void; onBack: () => void; onTags: () => void }) {
  ...
  const scheduleRead = useCallback(() => {
    ...
    readTimer.current = setTimeout(async () => {
      ...
      try {
        const result = await request<{ readSequence: number }>(`/api/admin/conversations/${id}/read`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sequence }), signal: controller.signal });
  ...
  useEffect(() => {
    ...
    const poller = createPoller(cursor.current,
      (after, signal) => request<MessageBatch & { conversation: ConversationDTO }>(`/api/admin/conversations/${id}/messages?after=${after}`, { signal }),
      (batch, next) => {
        if ("conversation" in batch) { const metadata = batch.conversation as ConversationDTO; setConversation(metadata); onUpdate(metadata); }
  ...
  }, [expired, id, onUpdate, scheduleRead, sessionFailure]);
  useLayoutEffect(() => {
