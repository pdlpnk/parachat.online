...
export function AdminWorkspace({displayName}:{displayName:string}){
 ...
 <aside className="admin-sidebar" aria-label="Список диалогов"><header className="admin-toolbar"><BrandMark small/><h1>LINA</h1><span className="admin-name">{displayName}</span><button disabled={logoutBusy} onClick={async()=>{if(logoutBusy)return;setLogoutBusy(true);try{await adminRequest('/api/admin/logout',jsonRequest({}));router.replace('/admin/login');router.refresh();}catch(e){if(e instanceof AdminRequestError&&e.status===401){router.replace('/admin/login');router.refresh();}else setError('Не удалось выйти. Повторите.');}finally{setLogoutBusy(false);}}}>Выйти</button></header>
 ...
 {expired?<p role="alert" className="admin-error">Сессия завершена. <a href="/admin/login">Войти снова</a></p>:error&&<p role="alert" className="admin-error">{error} <button onClick={reload}>Повторить</button></p>}
